...
void CClient::SendRequiredPasswordNotice() {
	PutClient(":layerbnc.org 464 " + GetNick() + " :Password required");
	PutClient(":layerbnc.org NOTICE AUTH :*** "
...
void CClient::ReadLine(const CString& sData) {
	...
	if (sCommand.Equals("ZNC") || sCommand.Equals("BNC")) {
		...
		return;
	} else if (sCommand.Equals("LAYERBNC")) {
                CString sTarget = sLine.Token(1);
                CString sModCommand;
		...
		if (sLine.length() >= 5)
			PutClient(":layerbnc.org PONG layerbnc.org " + sLine.substr(5));
		else
			PutClient(":layerbnc.org PONG layerbnc.org");
...
void CClient::RefuseLogin(const CString& sReason) {
	...
	PutClient(":layerbnc.org 464 " + GetNick() + " :" + sReason);
...
void CClient::PutModNotice(const CString& sModule, const CString& sLine) {
	...
	DEBUG("(" << GetFullName() << ") ZNC -> CLI [:" + m_pUser->GetStatusPrefix() + ((sModule.empty()) ? "status" : sModule) + "!znc@layerbnc.org NOTICE " << GetNick() << " :" << sLine << "]");
	Write(":" + m_pUser->GetStatusPrefix() + ((sModule.empty()) ? "status" : sModule) + "!znc@layerbnc.org NOTICE " + GetNick() + " :" + sLine + "\r\n");
}
...
void CClient::PutModule(const CString& sModule, const CString& sLine) {
	...
	DEBUG("(" << GetFullName() << ") ZNC -> CLI [:" + m_pUser->GetStatusPrefix() + ((sModule.empty()) ? "status" : sModule) + "!znc@layerbnc.org PRIVMSG " << GetNick() << " :" << sLine << "]");

	sLine.Split("\n", vsLines);
	for (it = vsLines.begin(); it != vsLines.end(); ++it) {
		Write(":" + m_pUser->GetStatusPrefix() + ((sModule.empty()) ? "status" : sModule) + "!znc@layerbnc.org PRIVMSG " + GetNick() + " :" + (*it) + "\r\n");
	}
}
...
{
	PutClient(":layerbnc.org CAP " + GetNick() + " " + sResponse);
}
...
void CClient::HandleCap(const CString& sLine)
{
	...
	if (sSubCmd.Equals("LS")) {
		...
		RespondCap("LS :" + sRes + "userhost-in-names multi-prefix layerbnc.org/server-time-iso layerbnc.org/batch layerbnc.org/self-message");
		m_bInCap = true;
		...
		for (it = vsTokens.begin(); it != vsTokens.end(); ++it) {
			...
			bool bAccepted = ("multi-prefix" == sCap) || ("userhost-in-names" == sCap) || ("layerbnc.org/server-time-iso" == sCap) || ("layerbnc.org/batch" == sCap) || ("layerbnc.org/self-message" == sCap);
		...
		for (it = vsTokens.begin(); it != vsTokens.end(); ++it) {
			...
			if ("multi-prefix" == *it) {
				...
				m_bUHNames = bVal;
			} else if ("layerbnc.org/server-time-iso" == *it) {
				m_bServerTime = bVal;
			} else if ("layerbnc.org/batch" == *it) {
				m_bBatch = bVal;
			} else if ("layerbnc.org/self-message" == *it) {
				m_bSelfMessage = bVal;
			}
		...
		if (m_bServerTime) {
			m_bServerTime = false;
			ssRemoved.insert("layerbnc.org/server-time-iso");
		}
		if (m_bBatch) {
			m_bBatch = false;
			ssRemoved.insert("layerbnc.org/batch");
		}
		if (m_bSelfMessage) {
			m_bSelfMessage = false;
			ssRemoved.insert("layerbnc.org/self-message");
		}
		CString sList = "";
		...
		PutClient(":layerbnc.org 410 " + GetNick() + " " + sSubCmd + " :Invalid CAP subcommand");
	}
}
